...
mod tests {
    ...
    fn parses_supported_slash_commands() {
        ...
        assert_eq!(
            SlashCommand::parse("/recipe mediation stress -> coping -> burnout"),
            Some(SlashCommand::Recipe {
                action: Some("mediation".to_string()),
    ...
    #[test]
    fn installs_example_regression_plugin_from_repo() {
        let config_home = temp_dir("example-plugin-home");
        let source_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("../../../examples/external-plugins/research-regression");
        assert!(
            source_root
                .join(".claw-plugin")
                .join("plugin.json")
        ...
        .expect("installing example plugin should succeed");
        assert!(install.reload_runtime);
        assert!(install
            .message
            .contains("installed research-regression@external"));

        let list = handle_plugins_slash_command(Some("list"), None, &mut manager)
            .expect("listing example plugin should succeed");
        assert!(list.message.contains("research-regression"));
        assert!(list.message.contains("enabled"));
