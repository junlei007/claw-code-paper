...
fn unix_timestamp_millis() -> u64 {
    ...
    .expect("unix timestamp in millis should fit into u64")
}
...
mod tests {
    ...
    async fn create_session(client: &Client, server: &TestServer) -> CreateSessionResponse {
        ...
        loop {
            let last_error = match client.post(server.url("/sessions")).send().await {
            ...
            assert!(
                ...
                "create request should return success: {last_error}"
            );
